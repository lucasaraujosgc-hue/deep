...
const PendenciesTab: React.FC = () => {
  ...
  const [selectedCompany, setSelectedCompany] =
    useState<CompanyPendency | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [sortOrder, setSortOrder] = useState<
    "status_desc" | "status_asc" | "name_asc"
  >("name_asc");
  ...
  const handleUpload = async (
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(
          "Erro ao processar arquivo(s): " + (data.error || "Desconhecido"),
        );
      }
    } catch (e) {
      console.error(e);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Mapeie os relatórios da Receita Federal usando IA para extrair
            pendências.
          </p>
        </div>
                      ...
                      <div className="font-medium text-slate-800 flex items-center gap-2">
                        {company.unmapped && (
                          <AlertTriangle
                            ...
                            title="Empresa não localizada no banco, precisa associar."
                          />
                        )}
                        {company.name}
                      </div>
                      <div className="text-xs text-slate-500">
                        {company.docNumber || "Sem CNPJ"}
                      </div>
                    </td>
                    <td className="px-6 py-4">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {selectedCompany.pendencies.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-slate-500">
                    Nenhuma irregularidade fiscal ou débito foi encontrada no
                    último relatório mapeado pela IA.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  {Object.entries(groupedPendencies).map(
                    ([type, group], idx) => (
                      <div
                        key={idx}
                          ...
                          {group.items.map((pend, i) => (
                            ...
                            >
                              ...
                              <span className="text-sm font-mono text-slate-800">
                                {pend.value
                                  ? formatValue(parseValue(pend.value))
                                  : "-"}
                              </span>
                            </div>
                          ))}
              ...
              <div className="mt-8 text-xs text-slate-400 flex justify-end items-center gap-1.5">
                ...
                Atualizado pelo último PDF inserido em:{" "}
